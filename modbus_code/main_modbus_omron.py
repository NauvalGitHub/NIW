...
from lib import omron_KM50C1FLK as km50
...
stopbits        = 1
parity          = 'E'
baudrate        = 19200   # data/byte transmission speed (in bytes per second)
...
timeout         = 20/1000   # the maximum time the master/client will wait for response from slave/server (in seconds)
...
def setup_modbus():
    ...
    #ct3 = msystem.node(unit=1, name='MSYSTEM', client=client, delay=client_latency, max_count=20, increment=2, shift=2)
    ct4 = km50.node(unit=4, name='KM50', client=client, delay=client_latency, max_count=20, increment=1, shift=0)
    #server = [ct1,ct2,ct3]
    server = [ct4]
    return server
...
def read_modbus(server):
    #return
    addr=[["Voltage_1","Current_1","Power_Factor","Frequency","Active_Power","React_Power","Temperature"]]
...
def update_database(server, timer):
    ...
    title = ["dataentered","RPi_Temp","Volt1","Current1","PF","Freq","Watt","VAr","Temp"]
    ...
    data = [timer.strftime("%Y-%m-%d %H:%M:%S"), cpu_temp, server[0].Voltage_1, server[0].Current_1, server[0].Power_Factor,
            server[0].Frequency, server[0].Active_Power,server[0].React_Power, server[0].Temperature]
